...
if [ -z $FUSION_EX ]; then
    FUSION_EX=~/dev/research/depth_fusion/fusion/src/build/depth_fusion
    echo "Depth fusion executable path set to default value '${FUSION_EX}'..."
fi
if [ -z $DATA_DIR ]; then
    DATA_DIR=~/Data/fusion/dtu/all_samples/
...
do
    ...
    # run depth fusion
    $FUSION_EX ${DATA_DIR}${fusion_scan_dir} 5

    # merge output point clouds
    python3 merge_depth_maps.py ${DATA_DIR}${fusion_scan_dir}points_mvs/

    # move merged point cloud to evaluation path
    cp ${DATA_DIR}${fusion_scan_dir}points_mvs/merged.ply ${EVAL_PC_DIR_FUSION}${pc_file_name_fusion}

    #### MVSNet original maps eval ####
    # merge output point clouds
    python3 merge_depth_maps.py ${DATA_DIR}${fusion_scan_dir}points_unfused/

    # move merged point cloud to evaluation path
    cp ${DATA_DIR}${fusion_scan_dir}points_unfused/merged.ply ${EVAL_PC_DIR_MVSNET}${pc_file_name_mvsnet}
done
