...
def load_pfm(file):
    ...
    scale = None
    data_type = None
    header = file.readline().decode('iso8859_15').rstrip()
    ...
    dim_match = re.match(r'^(\d+)\s(\d+)\s$', file.readline().decode('iso8859_15'))
    if dim_match:
        width, height = map(int, dim_match.groups())
    else:
        raise Exception('Malformed PFM header.')
    # scale = float(file.readline().rstrip())
    scale = float((file.readline()).decode('iso8859_15').rstrip())
...
def gen_dtu_resized_path(dtu_data_folder, mode='training'):
    ...
    training_set = [2, 6, 7, 8, 14, 16, 18, 19, 20, 22, 30, 31, 36, 39, 41, 42, 44,
                    45, 46, 47, 50, 51, 52, 53, 55, 57, 58, 60, 61, 63, 64, 65, 68, 69, 70, 71, 72]
    validation_set = [3, 5, 17, 21, 28, 35, 37, 38, 40, 43]
...
def gen_pipeline_mvs_list(dense_folder, scan_dir=None):
    """ mvs input path list """
    image_folder = os.path.join(dense_folder, scan_dir, "images")
    cam_folder = os.path.join(dense_folder, scan_dir, "cams")
